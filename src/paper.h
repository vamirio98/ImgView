...
namespace img_view
{
	...
	class Paper : public QWidget
	{
	  ...
	  public:
		 ...
		 *
		 * If the image size is smaller than the display area, show it in
		 * original size; else scale it to the display area size while keeping
		 * its aspect.
		 */
		void limitToWindow();
	  ...
	  private:
		/**
		 * @brief Adjust the scroll bar position according to the image size,
		 * keep the display area is the same part of the image
		 *
	  ...
	  private:
		...
		/* Scale factor which control the image size, the actual size of an
		 * image is (originSize * _initScaleFactor * _scaleFactor).
		 */
