 ...
 *
 * NOTE: this function will change the data stream status, you should save
 * the status yourself if you want to use it after calling this function.
 *
 * @return True when they are same
 */
bool compare(
    QDataStream& data, const qint64& offset, const qint64& len,
...
)
{
	...
	bool res =
	    data.readRawData(magic, len) != -1 && memcmp(magic, buf, len) == 0;
	delete[] magic;
...
ImageInfo::ImageInfo(const ImageInfo& rhs)
    : _hasBasicInfo(rhs._hasBasicInfo), _hasDetailInfo(rhs._hasDetailInfo),
      _size(rhs._size), _lastModified(rhs._lastModified), _format(rhs._format),
...
ImageInfo::ImageInfo(ImageInfo&& rhs) noexcept
    : _hasBasicInfo(rhs._hasBasicInfo), _hasDetailInfo(rhs._hasDetailInfo),
      _path(rhs._path), _filename(rhs._filename), _extension(rhs._extension),
      _size(rhs._size), _lastModified(rhs._lastModified), _format(rhs._format),
      _width(rhs._width), _height(rhs._height), _depth(rhs._depth)
{
...
QString ImageInfo::filename() const { return _filename; }

QString ImageInfo::extension() const { return _extension; }

ImageFormat ImageInfo::format() const { return _format; }

qint64 ImageInfo::size() const { return _size; }

int ImageInfo::width() const { return _width; }

int ImageInfo::height() const { return _height; }

QSize ImageInfo::dimensions() const { return QSize(_width, _height); }

qint64 ImageInfo::pixels() const { return _width * _height; }

qint64 ImageInfo::lastModified() const { return _lastModified; }
