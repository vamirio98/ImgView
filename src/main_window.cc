...
namespace img_view
{
	...
	void MainWindow::init()
	{
		...
		for (int i = 0; i != gOpt.recentBooks().capacity(); ++i) {
			...
			connect(
			    action, &QAction::triggered, this, &MainWindow::onRecentBooks
			);
	...
	void MainWindow::setupSlots()
	{
		...
		connect(
		    _ui->_openFileLoc, &QAction::triggered, this,
		    &MainWindow::onOpenFileLoc
		);
		connect(_ui->_exit, &QAction::triggered, this, &MainWindow::onExit);

		connect(&gOpt, &Options::showChanged, this, &MainWindow::toggleClose);
		connect(
		    &gOpt, &Options::hasHistoryChanged, this,
		    &MainWindow::toggleRecentBooks
		);
		connect(
		    &gOpt, &Options::showChanged, this, &MainWindow::toggleOpenFileLoc
		);
